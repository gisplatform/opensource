/*!
 * \file treeview_dnd_test.c
 *
 * \author Pierre-Louis Malatray.
 * \date 29.09.2014
 * \brief Исходный файл с примером реализации D'n'D в рамках одного GtkTreeView.
 *
*/
...
GtkWidget *add_treeview(GtkWidget *box, struct DATA array[])
{
  ...
  g_signal_connect(tree_model,"row-deleted", G_CALLBACK(on_drag_data_deleted), NULL);
  // Attach a "drag-data-inserted" signal to start DnD operation
  g_signal_connect(tree_model, "row-inserted", G_CALLBACK(on_drag_data_inserted), NULL);
...
int main(int argc, char **argv)
{
  ...
  /* Create treeview */
  //GtkWidget *view1 = add_treeview(hbox, row_data);
  add_treeview(hbox, row_data);
